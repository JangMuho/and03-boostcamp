...
fun AddQuoteDialog(
    ...
    And03ActionDialog(
        ...
        content = {
            And03InfoSection(
                title = stringResource(id = R.string.add_quote_info_title),
                description = stringResource(id = R.string.add_quote_info_description)
            )

            QuoteInputSection(quoteState = quoteState)
...
private fun QuoteInputSection(
    ...
    Column(
        ...
        Row(verticalAlignment = Alignment.Bottom) {
            Text(
                text = stringResource(id = R.string.add_quote_sentence_label),
                style = And03Theme.typography.labelLarge
            )

            Spacer(modifier = Modifier.weight(1f))

            AddTextByImageButton(onAddByImageClick = { isOCRBottomSheetVisible = true })
        }
